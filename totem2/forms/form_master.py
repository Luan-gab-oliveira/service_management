...
try:
    ...
    class Application(func):
        ...
        def widgets(self):
            ###Botões###
            self.photo_bt_CSUS = PhotoImage(file = bt[0]) #Imagem botão Cartão SUS
            ...
            self.bt_SUS.place(anchor= 'center',relx= 0.25, rely=0.6, width= 425, height= 160) #Localizando o botão na tela

            self.photo_bt_TFD = PhotoImage(file = bt[1]) #Imagem botão TFD
            ...
            self.bt_TFD.place(anchor= 'center',relx= 0.75, rely=0.6, width= 425, height= 160) #Localizando o botão na tela
