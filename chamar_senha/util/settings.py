...
config = ConfigParser()
configFile = fr'{get_dir}\util\settings.ini'
...
class Functionsconfig():
    def salvar_config(self):
        ...
        if messagebox.askyesno('Configurações','Tem certeza que deseja salvar estás alterações?') == YES:
            ...
            config.set('server','password', self.entry_pass.get())
    ...
    def loadConfigFile(self):
        ...
        self.entry_pass.insert(0,server_config['password'])
...
class ConfigGUI(Functionsconfig):
    ...
    def make_widgets(self):
        ...
        self.label.place(relwidth=0.5, relheight=0.08, rely=0.1)
        self.entry_servidor = Entry(self.frame)
        self.entry_servidor.config(font=(0,12),justify='left')
        self.entry_servidor.place(relwidth=0.5, relheight=0.08,relx=0.5, rely=0.1)

        self.label = Label(self.frame, text='Username:')
        self.label.config(font=(0,12),anchor='w',)
        self.label.place(relwidth=0.5, relheight=0.08, rely=0.22)
        self.entry_username = Entry(self.frame)
        self.entry_username.config(font=(0,12),justify='left')
        self.entry_username.place(relwidth=0.5, relheight=0.08,relx=0.5, rely=0.22)

        self.label = Label(self.frame, text='Database:')
        self.label.config(font=(0,12),anchor='w',)
        self.label.place(relwidth=0.5, relheight=0.08, rely=0.34)
        self.entry_tabela = Entry(self.frame)
        self.entry_tabela.config(font=(0,12),justify='left')
        self.entry_tabela.place(relwidth=0.5, relheight=0.08,relx=0.5, rely=0.34)

        self.label = Label(self.frame, text='Setor:')
        self.label.config(font=(0,12),anchor='w',)
        self.label.place(relwidth=0.5, relheight=0.08, rely=0.46)
        self.entry_setor = Entry(self.frame)
        self.entry_setor.config(font=(0,12),justify='left')
        self.entry_setor.place(relwidth=0.5, relheight=0.08,relx=0.5, rely=0.46)

        self.label = Label(self.frame, text='Password:')
        self.label.config(font=(0,12),anchor='w',)
        self.label.place(relwidth=0.5, relheight=0.08, rely=0.58)
        self.entry_pass = Entry(self.frame)
        self.entry_pass.config(font=(0,12),justify='left', show='*')
        self.entry_pass.place(relwidth=0.5, relheight=0.08,relx=0.5, rely=0.58)

        self.btn_salvar = Button(self.frame, text='Salvar')
        self.btn_salvar.place(relwidth=0.5, relheight=0.12,relx=0.5, rely=0.76)
        self.btn_salvar.config(cursor='hand2', command=self.salvar_config)
